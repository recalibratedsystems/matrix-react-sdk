...
export default class SecureBackupPanel extends React.PureComponent<{}, IState> {
    ...
    public componentWillUnmount(): void {
        ...
        if (MatrixClientPeg.safeGet()) {
            MatrixClientPeg.safeGet().removeListener(CryptoEvent.KeyBackupStatus, this.onKeyBackupStatus);
            MatrixClientPeg.safeGet().removeListener(
    ...
    public render(): React.ReactNode {
        ...
        if (error) {
            ...
            let backupSigStatuses: React.ReactNode | undefined = backupSigStatus?.sigs?.map((sig, i) => {
                ...
                const fromThisUser = sig.crossSigningId && sig.deviceId === MatrixClientPeg.safeGet().getCrossSigningId();
                let sigStatus;
